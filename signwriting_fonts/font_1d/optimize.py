...
def _fit_circle_lsq(points, max_gap_deg=MAX_ANGULAR_GAP_DEG):
    """Algebraic LSQ fit of (x-cx)² + (y-cy)² = R² to `points`.

    Rejects (returns None) when any consecutive-anchor angular gap around
    the fitted centre exceeds `max_gap_deg` — that filters out arc
    fragments that happen to lie close to a circle locally but aren't
    actually closed rings (e.g. the outer outline of a comb shape). The
    bezier anchors of a SignWriting "circle" sit ON the true circle (with
    control points pushed outward to bulge each segment), so LSQ gives a
    clean center+radius and very small residuals when the shape really
    is a circle.

    Returns (cx, cy, R, max_relative_error).
    """
    if len(points) < 4:
        return None
    ...
    if float(np.max(gaps)) > max_gap_deg:
        return None
...
def _fit_axis_aligned_ellipse(points):
    """Fit a circle to anchors. Returns (cx, cy, rx, ry, max_rel_err) with
    rx == ry, or None if the points aren't ring-like."""
    if len(points) < 5:
        return None
...
def optimize_path_d(d: str):
    ...
    out_subpaths = []
    n_replaced = 0
    for segments in parse_subpaths(d):
        pts = _anchors(segments)
        fit = _fit_axis_aligned_ellipse(pts)
        if fit is not None and fit[4] < ELLIPSE_TOLERANCE:
            # Detection uses the anchor LSQ (passes when the shape is truly
            # a circle). For emission, size the synthetic ellipse to the
            # source sub-path's actual bbox — SignWriting source control
            # points bulge past their anchors, so an anchor-radius ellipse
            # renders ~2.5% smaller than the original.
...
# decoration). Accepts "circle-shaped" sub-paths that the strict optimizer
# rejects — clearly-circular rings with sparse anchor spacing (S21600: 6
# anchors at 90° max-gap) or slight wobble (S2ff10: ~6% residual). Gaps
# above 90° still get rejected (lumpy arcs no matter how well the fit
# looks — see _prim_08301's 97° gap).
...
def count_circles_in_svg(svg_text: str) -> int:
    ...
    for m in _PATH_D.finditer(svg_text):
        for segments in parse_subpaths(m.group(2)):
            fit = _fit_circle_lsq(_anchors(segments),
                                  max_gap_deg=DETECT_MAX_ANGULAR_GAP_DEG)
            if fit is not None and fit[3] < DETECT_TOLERANCE:
                n += 1
    return n
